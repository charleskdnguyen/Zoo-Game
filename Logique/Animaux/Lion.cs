...
namespace TP2.Logique
{
    ...
    {
        /// <summary>
        /// Constructeur pour créer un lion
        /// </summary>
        /// <param name="X"></param>
        /// <param name="Y"></param>
        ...
        public Lion(int X, int Y, Genre genre, Vieillesse vieillesse, Etat etat, Bitmap[,] tab, TypeAnimal type, List<Animal> listeAnimal, Hero Hero) : base(X, Y, Genre.Male, Vieillesse.Adulte, Etat.Nourrit, tab, TypeAnimal.Lion,listeAnimal,Hero)
        {
            _etat = etat;
            _genre = genre;
            _vieillesse = vieillesse;
            _type = type;
            tabTypeAnimal = DeterminerAnimal(TypeAnimal.Lion);
        }
